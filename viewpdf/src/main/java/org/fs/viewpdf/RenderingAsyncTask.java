...
class RenderingAsyncTask extends AsyncTask<Void, PagePart, Void> {
  ...
  public void addRenderingTask(int userPage, int page, float width, float height, RectF bounds,
      boolean thumbnail, int cacheOrder, boolean bestQuality, boolean annotationRendering) {
    RenderingTask task =
        new RenderingTask(width, height, bounds, userPage, page, thumbnail, cacheOrder, bestQuality,
            annotationRendering);
  ...
  @Override protected void onProgressUpdate(PagePart... part) {
    PDFView cached = getCachedPdfView();
    if (cached != null) {
      cached.onBitmapRendered(part[0]);
    }
  }
  ...
  private PagePart proceed(RenderingTask renderingTask) {
      ...
      if (w <= 0 || h <= 0) return null; //do not waste resources

      if (render == null) {
        render = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);
      } else if (w <= render.getWidth() && h <= render.getHeight() && !render.isRecycled()) {//we can only reconfigure size is less or equal to previous ones
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
          render.reconfigure(w, h, Bitmap.Config.ARGB_8888);
        } else {
          //clear inside so we can use it again
          render.eraseColor(Color.TRANSPARENT);
        }
      } else {
        if (render != null && !render.isRecycled()) {
          render.recycle();
        }
      ...
      if (!isCancelled()) {
        pdfiumCore.renderPageBitmap(pdfDocument, render, renderingTask.page, roundedRenderBounds.left,
            roundedRenderBounds.top, roundedRenderBounds.width(), roundedRenderBounds.height(),
            renderingTask.annotationRendering);
      } else {
        return null;
      }

      Bitmap page = render != null ? (!renderingTask.bestQuality ? render.copy(Bitmap.Config.RGB_565, false)
          : render.copy(Bitmap.Config.ARGB_8888, false)) : null;

    if (page == null) return null;

      return new PagePart(renderingTask.userPage, renderingTask.page, page, //
  ...
  private PDFView getCachedPdfView() {
