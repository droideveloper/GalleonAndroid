...
import org.fs.viewpdf.exception.FileNotFoundException;
import org.fs.viewpdf.listener.OnDrawListener;
import org.fs.viewpdf.listener.OnErrorListener;
import org.fs.viewpdf.listener.OnLoadCompleteListener;
import org.fs.viewpdf.listener.OnPageChangeListener;
import org.fs.viewpdf.listener.OnPageScrollListener;
import org.fs.viewpdf.model.PagePart;
import org.fs.viewpdf.scroll.ScrollHandle;
import org.fs.viewpdf.util.ArrayUtils;
...
public class PDFView extends RelativeLayout {
  ...
  private void load(String path, boolean isAsset, String password, OnLoadCompleteListener listener,
      OnErrorListener onErrorListener) {
    load(path, isAsset, password, listener, onErrorListener, null);
  }

  private void load(String path, boolean isAsset, String password, OnLoadCompleteListener onLoadCompleteListener, OnErrorListener onErrorListener,
      int[] userPages) {
  ...
  public void setPositionOffset(float progress, boolean moveHandle) {
    if (swipeVertical) {
      moveTo(currentXOffset, (-getPageCount() * toCurrentScale(optimalPageHeight) + getHeight()) * progress,
          moveHandle);
    } else {
      moveTo((-getPageCount() * toCurrentScale(optimalPageWidth) + getWidth()) * progress,
          currentYOffset, moveHandle);
    }
    loadPageByOffset();
  }
  ...
  @Override protected void onSizeChanged(int w, int h, int oldw, int oldh) {
    ...
    loadPages();
    if (swipeVertical) moveTo(currentXOffset, calculateCenterOffsetForPage(currentFilteredPage));
    else moveTo(calculateCenterOffsetForPage(currentFilteredPage), currentYOffset);
  }
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    // Draws background, Color.WHITE was original
  ...
  private void drawPart(Canvas canvas, PagePart part) {
    ...
    if (swipeVertical) localTranslationY = toCurrentScale(part.getUserPage() * optimalPageHeight);
    else localTranslationX = toCurrentScale(part.getUserPage() * optimalPageWidth);
    canvas.translate(localTranslationX, localTranslationY);

    srcRect.set(0, 0, renderedBitmap.getWidth(), renderedBitmap.getHeight());
    ...
    dstRect.set((int) offsetX, (int) offsetY, (int) (offsetX + width), (int) (offsetY + height));
    //RectF dstRect =
