...
public class AdaptiveMap {
  ...
  /**
   * Normalizes Simply an image's background using default parameters.
   * @param pixs A source pix image.
   * @return the source pix imge with a normalized background
   */
  public static Pix backgroundNormSimple(Pix pixs) {
        if (pixs == null) {
            throw new IllegalArgumentException("Source pix must be non-null");
        }

        long nativePix = nativeBackgroundNormSimple(pixs.getNativePix());
        if(nativePix == 0) {
            throw new RuntimeException("Failed to normalize background.");
        }
    ...
    private static native long nativeBackgroundNormSimple(
            long nativePix);
