...
public class MorphApp {
    ...
    private final static int H_DEFAULT = 17;
    private final static int V_DEFAULT = 17;
    /**
    ...
    public static Pix pixTophat(Pix pixs, int hsize, int vsize, int type) {
      ...
      if (hsize < 1 || vsize < 1) throw new IllegalArgumentException("hsize or vsize < 1");
      if (type < 0 || type > 1) throw new IllegalArgumentException("Type must be L_TOPHAT_BLACK or L_TOPHAT_WHITE");

      long nativePix = nativePixTophat(pixs.getNativePix(), hsize, vsize, type);

      if (nativePix == 0) throw new RuntimeException("Failed to perform Tophat on image");

      return new Pix(nativePix);
    }

  /**
   * Tophat operation with default parameters.
   *
   * @param pixs A source image.
   * @return the source image result with tophat
   */
    public static Pix pixTophat(Pix pixs) {
      return pixTophat(pixs, H_DEFAULT, V_DEFAULT, L_TOPHAT_BLACK);
    }
  ...
  /**
   * Invert operation on pix
   *
   * @param pixs A source image.
   * @return the source image inverted.
   */
    public static Pix pixInvert(Pix pixs) {
      if (pixs == null) {
        throw new IllegalArgumentException("Source pix must be non-null");
      }
      nativePixInvert(pixs.getNativePix());
      return pixs;
    }
    ...
    private static native void nativePixInvert(long nativePix);
}
