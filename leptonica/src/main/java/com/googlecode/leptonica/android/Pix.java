...
public class Pix {
  ...
  /**
   * Store image pix with desired output format.
   *
   * @param filename file to save. (should be full path with extension)
   * @param format format of desired output
   * @return true if success false otherwise.
   */
  public boolean write(File filename, @IntRange(from = ImageFormat.Unknown, to = ImageFormat.Spix) int format) {
    if (mRecycled) {
    ...
    return nativeWrite(filename.getAbsolutePath(), mNativePix, format);
  }

  //added this method here because we do not want covertPdf collide in naming for convenience
  public Pix convertRGBToGray() {
    return Convert.convertRGBToGray(this);
  }
  ...
  private static native boolean nativeWrite(String filename, long nativePix, int format);
}
